...
> **Головне (перевірено сніфером):** Beam стрімить, лише поки приймач
> **безперервно пінгує** Beam RX (`192.168.1.21`). Пінг стих на кілька секунд —
> Beam глушить потік. Тому у `thermal_receiver.sh` є безперервний keepalive-пінг
> з авто-рестартом (через `sudo ping`, тож для `armsom` потрібен passwordless sudo).
