...
2. **вимикає USB-autosuspend** (часта причина спонтанної переенумерації камери);
3. чекає появи USB-камери (YUYV-нода, номер `/dev/videoN` «плаває»);
4. піднімає `/root/rv1106_sender --port 5000 --fps 60` у фоні (лог `/tmp/thermal.log`);
5. **WATCHDOG:** стежить за камерою — якщо вона переенумерувалась (змінився USB
   `devnum`) або sender впав, гасить старий процес і піднімає новий на свіжій ноді
   (лог `/tmp/thermal_watchdog.log`). Тобто рідкісний блип USB залічується сам.

Приймач на BPI (`reproduce/05_run_bpi_receiver.sh`) теж **самовідновний** — після
блипу перепідключається автоматично. Разом це дає «увімкнув → працює» без рук.
